...
import { locationData } from '../src/apiClient'
import * as testData from './testData'
import { createOpenWeatherURL } from '../src/weatherAPIClient'

chai.use(chaiAsPromised)

export const newYork: locationData = {
  index: 0,
...
describe('apiClient/newGeocodeData', function () {
  it('It should return a basic geocode object with just error and index fields when an empty object is passed in', function () {
    ...
    const result = apiClient.newGeocodeData({} as locationData)
    expect(result).to.not.be.undefined
